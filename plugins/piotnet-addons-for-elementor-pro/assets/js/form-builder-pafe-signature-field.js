...
(function ($) {
	...
	var WidgetPafeFormBuilderHandlerSignature = function ($scope, $) {

        var $elements = $scope.find('[data-pafe-signature] canvas');
		...
		$.each($elements, function (i, $element) {
			...
			var $pafeSignature = $($element).closest('[data-pafe-signature]'),
				$clearButton = $pafeSignature.find('[data-pafe-signature-clear]'),
				$exportButton = $pafeSignature.find('[data-pafe-signature-export]');
			...
			$exportButton.click(function(){
				if (signaturePad.isEmpty()) {
					$pafeSignature.find('.elementor-field').val('');
				} else {
					var url = signaturePad.toDataURL();
					$pafeSignature.find('.elementor-field').val(url);
				}
			});
    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        elementorFrontend.hooks.addAction('frontend/element_ready/pafe-form-builder-field.default', WidgetPafeFormBuilderHandlerSignature);
