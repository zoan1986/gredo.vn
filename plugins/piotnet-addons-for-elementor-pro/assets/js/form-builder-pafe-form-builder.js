								...
								if ($(this).val()=='' && $(this).attr('aria-required') == "true" && $(this).attr('data-pafe-form-builder-select-autocomplete') !== undefined || checked == 0 && $checkboxRequired.length > 0) {
									...
									if ( $(this).data('pafe-form-builder-image-select') != undefined ) {
										if ( $(this).closest('.pafe-image-select-field').find('.image_picker_selector').find('.selected').length < $(this).data('pafe-form-builder-image-select-min-select') ) {
											$(this).closest('.elementor-field-group').find('[data-pafe-image_select_min_select_check]').html('Please select a minimum <b>' + $(this).data('pafe-form-builder-image-select-min-select') + '</b> image(s)');
											error++;
										} else {
...
jQuery(document).ready(function($) {
	...
	$(document).on('change keyup paste', '[type="number"][data-pafe-form-builder-form-id]',function(e){
		var val = $(this).val();
	...
	function pafeLivePreview(field) {
		if ($(document).find('[data-pafe-form-builder-live-preview]').length > 0) {
			...
			if (fieldId !== undefined) {
				var $livePreview = $(document).find('[data-pafe-form-builder-live-preview="' + fieldId.replace('form-field-', '') + '"]');
